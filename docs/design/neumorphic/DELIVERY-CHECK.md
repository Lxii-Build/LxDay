...
- Android 已落地拟态表面封装、语义按钮/弹窗复用、统一播放器 mini/full 外壳、SAF ZIP 本机导入与已发布资源下载、主页陪伴角入口及设置页管理入口；页面卡片已按装饰面／可点击面区分。Live2D 原生 Cubism Core/renderer、真实模型导入和真机绘制仍是明确未验收项，不能把占位预览当成功证据。
- Live2D 管理页另已落地本机“当前使用”选择与安全回退：首次导入自动选中、切换不触网、删除当前模型后清理失效偏好；主页只展示本机当前名，不伪称已经完成 Cubism 绘制。
