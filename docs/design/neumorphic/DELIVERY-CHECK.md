...
- 交互表面残留已继续收口：可点击/长按卡片使用共享 Inset 按压反馈并移除平台 ripple；主页待办 FAB、相册详情照片网格、本机选图网格均走语义拟态组件，选图保留 Checkbox 状态语义。JDK 21 + Gradle 9.7 下 `:app:compileDebugKotlin`、`:app:testDebugUnitTest`、`:app:lintDebug` 通过。
