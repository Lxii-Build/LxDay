...
// ================= 按来源 IP 的限流 =================
//
// ★★ 为什么必须有这一层：APP_KEY 不是秘密 ★★
//
// APP_KEY 通过 `BuildConfig` 编进 APK，任何人 `jadx` 反编译就能拿到明文。
// 这不是本项目的实现缺陷 —— **任何随客户端分发的密钥都不可能保密**，
// 客户端上没有能藏住秘密的地方。
//
// 所以 APP_KEY 的真实作用只有一个：把「顺手扫全网的爬虫/脚本」挡在门外
//（它们不会为一个私有 App 去逆向）。它**不构成**任何安全边界。
// 正确的威胁模型是：**假设攻击者已持有合法 APP_KEY 与合法 JWT**，
...
var (
	// 注册：**最要紧的一条**。
	//
	// 攻击链的第一步就是"用 APP_KEY 调注册接口拿 JWT"，而注册此前
