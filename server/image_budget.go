...
// （JPEG 压得越狠，同样 20MB 的文件解出来越大）。上限 MaxPixels 是 64M 像素，
// 一张 PNG 解成 RGBA 就是 64M × 4B ≈ **256MB**；而解出来的 src 要一直活到
// 缩略图与预览图都写完（storeMediaFile 里连做两次 writeFit）。
//
...
//
// 3 是按「单张最坏 256MB × 3 ≈ 768MB 峰值」估的，留给小规格 VPS 一点余量。
