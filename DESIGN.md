...
└─ UI层:  4 Tab (此刻/待办/日记/我的) + 通知卡 XML + 深色模式
   服务端 (Go + Gin + MySQL + Redis, 香港轻量云)
├─ REST API: 认证/绑定/待办/日记/状态历史/图片上传(本地磁盘)
├─ WebSocket Hub: 双人房间, 在线直转, 离线补偿队列(Redis), 低电量/指定WiFi事件
├─ 定时任务: 待办到点提醒扫描, 历史状态聚合落库兜底
└─ 存储: MySQL(用户/绑定/待办/日记/状态历史/用量) + Redis(在线/最新状态/事件队列) + 本地磁盘(图片)
```

**数据隔离**：所有业务数据（待办/日记/状态历史）以 `pair_id` 为隔离键，仅对 pair 内双方可见。
...
- 发布 / 编辑 / 删除 / 按日期归档，文字 + 图片（多图）。
- 图片上传：客户端 POST → 服务端存本地磁盘 `uploads/diary/{pairId}/{date}/{uuid}.jpg`，返回可访问 URL；Nginx 静态服务 `/uploads/`。
- 新日记发布 → 对方收到 WS `diary_new` → 本地通知。
- 数据：`diary` + `diary_image` 两表，按 `pair_id` 隔离。
